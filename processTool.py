...
class Process:
	...
	def hexDump(self, address: int, length: int = 32) -> bytes | None:
		...
		for offset in range(0, len(data), 16):
			...
			print(f"{address + offset:016X}  {hexValues:<47}  {asciiValues}")
			chunks.append(chunk)
	...
	def getModuleSize(self) -> int:
		return self.getModule().SizeOfImage
	...
	def getModule(self) -> Any:
		...
		if module is None:
			raise RuntimeError(f"Could not find module: {self.PROCESS_NAME}")
	
		return module

	
	def getModuleBase(self) -> int:
	...
	def tracePointer(
		...
		currentAddress = self.readLongLong(startOffset)
	...
	def readPointer(
		...
		finalAddress = self.resolvePointer(startOffset, offsets)
